...
async function executeAdd(server, options) {
  ...
  if (options.listTemplates) {
    ...
    for (const name of templates) {
      ...
      if (template.command) {
        console.log(
          `    Command: ${template.command} ${(template.args || []).join(' ')}`
        );
      }
      console.log('');
    }
  ...
  if (!server) {
    ...
    console.log(
      '  aios mcp add myserver --command npx --args "-y,@scope/mcp-server"'
    );
    process.exit(1);
  }
  ...
  const isToggleOperation =
    options.enable ||
    (options.disable && !options.type && !options.url && !options.command);
  if (isToggleOperation) {
    const enabled = Boolean(options.enable);
    console.log(
      `${enabled ? 'Enabling' : 'Disabling'} server "${server}"...\n`
    );
  ...
  if (result.success) {
    ...
    if (template && !serverConfig) {
      ...
      if (template.env) {
        const envVars = Object.keys(template.env).filter((k) =>
          template.env[k].startsWith('${')
        );
        if (envVars.length > 0) {
    ...
    if (result.error.includes('No template available')) {
      ...
      console.log(
        `  aios mcp add ${server} --command npx --args "-y,@scope/server"`
      );
    }
