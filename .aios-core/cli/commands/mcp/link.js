...
async function executeLink(options) {
  ...
  if (status.status === LINK_STATUS.DIRECTORY) {
    if (analysis.projectConfig.found) {
      ...
      console.log(
        `  ${analysis.projectConfig.serverCount} servers configured\n`
      );
      ...
      if (options.migrate) {
        ...
        if (migrationResult.success) {
          console.log(
            `  ✓ Migrated ${migrationResult.results.serversMigrated} servers`
          );
          if (migrationResult.results.backupPath) {
            console.log(
              `  ✓ Backup created at ${migrationResult.results.backupPath}`
            );
          }
        ...
        if (migrationResult.success) {
          console.log(
            `  ✓ Added ${migrationResult.results.serversMigrated} new servers`
          );
        ...
        console.log(
          '  3. --force    Overwrite project config (creates backup)'
        );
        console.log('');
