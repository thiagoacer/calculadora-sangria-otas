...
function loadLicenseModules() {
  try {
    ...
    const { ProFeatureError, LicenseActivationError } = require(
      path.join(licensePath, 'errors')
    );
...
async function activateAction(options) {
  ...
  try {
    ...
    if (!writeResult.success) {
      console.error(
        `Warning: Could not save license cache: ${writeResult.error}`
      );
    }
    ...
    console.log(
      `  Seats:        ${result.seats.used}/${result.seats.max} used`
    );
    console.log(`  Valid until:  ${formatDate(result.expiresAt)}`);
    console.log(
      `  Cache:        ${result.cacheValidDays} days offline operation`
    );
    console.log('');
  } catch (error) {
...
function statusAction() {
  const { featureGate, readLicenseCache, maskKey, hasPendingDeactivation } =
    loadLicenseModules();
  ...
  if (cache.activatedAt) {
    ...
    const expiryDate = new Date(
      activatedDate.getTime() + cacheValidDays * 24 * 60 * 60 * 1000
    );
  ...
  if (pending && pending.pending) {
    ...
    console.log(
      '  This will be synced on next online activation or validation.'
    );
  }
...
async function deactivateAction(options) {
  ...
  if (!options.force) {
    ...
    console.log(
      '  - Disable all Pro features (Core features remain available)'
    );
...
function featuresAction() {
  ...
  const total = Object.values(byModule).reduce(
  ...
  if (options.verify) {
    ...
    try {
      ...
      if (deps['@aios-fullstack/pro']) {
        console.log(
          `✅ @aios-fullstack/pro@${deps['@aios-fullstack/pro'].version} is installed`
        );
      } else {
...
function createProCommand() {
  const proCmd = new Command('pro').description('AIOS Pro license management');
