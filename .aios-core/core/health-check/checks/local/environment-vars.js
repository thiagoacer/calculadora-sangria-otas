  ...
  async execute(_context) {
    ...
    for (const varName of AIOS_VARS) {
      if (process.env[varName]) {
        aiosVars.push({
          name: varName,
          value: this.maskValue(process.env[varName]),
        });
      }
    }
    ...
    if (missingRequired.length > 0) {
      return this.fail(
        ...
        {
          recommendation:
            'Set the missing environment variables in your shell profile',
          details: {
            missingRequired,
            missingRecommended,
          },
    ...
    if (missingRecommended.length > 0) {
      return this.warning(
        ...
        {
          recommendation:
            'Consider setting recommended variables for full functionality',
          details: {
            setVars,
            missingRecommended,
            aiosVars,
