...
class BranchProtectionCheck extends BaseCheck {
  ...
  async execute(context) {
    ...
    try {
      ...
      const mainBranch = localBranches.find((b) =>
      ...
      if (onProtectedBranch) {
        return this.warning(
          `Currently on protected branch '${currentBranch}'`,
          {
            recommendation:
      ...
      if (!mainBranch) {
        return this.warning(
          'No standard main branch found (main/master/develop)',
          {
      ...
      return this.pass(
        `Working on branch '${currentBranch}', main branch is '${mainBranch}'`,
        {
          details,
        }
      );
