...
class IdeationEngine {
  constructor(config = {}) {
    ...
    // Output paths
    this.outputDir =
      config.outputDir || path.join(this.rootPath, '.aios', 'ideation');
  ...
  checkMissingCaching() {
    ...
    try {
      ...
      const lines = result
        .split('\n')
        .filter(
          (l) => l.trim() && !l.includes('node_modules') && !l.includes('Cache')
        );
  ...
  checkAccessibility() {
    ...
    try {
      ...
      const lines = result
        .split('\n')
        .filter(
          (l) => l.trim() && !l.includes('aria-') && !l.includes('node_modules')
        );
