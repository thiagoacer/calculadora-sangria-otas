...
class BobOrchestrator {
    ...
    try {
      ...
      if (!lockAcquired) {
        return {
          ...
          action: 'lock_failed',
          error:
            'Another Bob orchestration is already running. Wait or check .aios/locks/',
        };
      }
  ...
  async _handleNoConfig(_context) {
    ...
    return {
      ...
      data: {
        message:
          'Projeto sem configuração AIOS detectado. Iniciando onboarding...',
  ...
  async _handleBrownfield(context) {
    this._log(
      '🔍 First execution detected — project has code but no AIOS docs'
    );
