...
class WorkflowOrchestrator {
  ...
  constructor(workflowPath, options = {}) {
    ...
    this.options = {
      ...
      onPhaseComplete:
        options.onPhaseComplete || this._defaultPhaseComplete.bind(this),
  ...
  async loadWorkflow() {
    try {
      ...
      const workflowId =
        this.workflow.workflow?.id || path.basename(this.workflowPath, '.yaml');
  ...
  async preparePhase(phase) {
    ...
    if (phase.creates) {
      const creates = Array.isArray(phase.creates)
        ? phase.creates
        : [phase.creates];
      ...
      for (const action of phase.preActions) {
        try {
          ...
          if (action.blocking !== false) {
            throw new Error(
              `Pre-action failed: ${action.type} - ${error.message}`
            );
          }
        }
      }
    ...
    switch (action.type) {
      ...
      case 'file_exists': {
        const exists = await fs.pathExists(
          path.join(this.options.projectRoot, action.path)
        );
      ...
      default:
        console.log(
          chalk.yellow(`   ⚠️ Unknown pre-action type: ${action.type}`)
        );
        return { success: true };
    }
  }
  ...
  async execute() {
    ...
    console.log(
      chalk.gray(`   Parallel phases: ${parallelPhases.join(', ') || 'None'}`)
    );
    ...
    console.log(
      chalk.gray(
        `   📝 TypeScript: ${this.techStackProfile.hasTypeScript ? '✓' : '✗'}`
      )
    );
    console.log(
      chalk.gray(
        `   📋 Applicable phases: ${this.techStackProfile.applicablePhases.join(', ')}`
      )
    );
    console.log(
      chalk.gray(`   🎯 Confidence: ${this.techStackProfile.confidence}%`)
    );
  ...
  _groupPhases(sequence, parallelPhases) {
    ...
    for (const phase of sequence) {
      ...
      if (
        isParallel !== currentGroup.parallel &&
        currentGroup.phases.length > 0
      ) {
  ...
  async _executeParallelPhases(phases) {
    ...
    const phasePromises = phases.map((phase) =>
      this._executeSinglePhase(phase)
    );
    ...
    results.forEach((result, index) => {
      if (result.status === 'rejected') {
        console.log(
          chalk.red(`   Phase ${phases[index].phase} failed: ${result.reason}`)
        );
      }
    });
  ...
  async _executeSinglePhase(phase) {
    ...
    if (phase.condition) {
      ...
      if (!conditionResult.shouldExecute) {
        ...
        const skipResult = this.skillDispatcher.createSkipResult(
          phase,
          skipReason
        );
    ...
    if (phase.requires) {
      ...
      if (missingDeps.length > 0) {
        console.log(
          chalk.yellow(
            `   ⚠️  ${phaseName}: Missing dependencies: ${missingDeps.join(', ')}`
          )
        );
  ...
  _evaluateConditionLegacy(condition) {
    ...
    if (typeof condition === 'object') {
      ...
      const fieldValue =
        this.contextManager?.getPreviousPhaseOutputs()?.[field];
  ...
  _projectHasDatabase() {
    ...
    return (
      ...
      (fs.existsSync(envPath) &&
        fs.readFileSync(envPath, 'utf8').includes('SUPABASE'))
    );
  }
  ...
  _generateExecutionSummary() {
    ...
    return {
      ...
      status:
        this.executionState.failedPhases.length === 0
          ? 'completed'
  ...
  _defaultPhaseStart(phase) {
    ...
    console.log(
      chalk.gray(`   Agent: @${phase.agent} | Action: *${phase.action}`)
    );
  }
  ...
  async resumeFrom(fromPhase) {
    ...
    const sequence = this.workflow.sequence || [];
    const remainingPhases = sequence.filter(
      (p) => p.phase >= fromPhase && !p.workflow_end
    );
