...
class BackupManager {
  ...
  async initialize() {
    try {
      ...
      console.error(
        chalk.red(`Failed to initialize backup system: ${error.message}`)
      );
    }
  }
    ...
    try {
      ...
      for (const file of files) {
        ...
        try {
          ...
          console.warn(
            chalk.yellow(`Warning: Could not backup ${file}: ${error.message}`)
          );
        }
      }
      ...
      console.log(
        chalk.gray(
          `   Files: ${files.length}, Size: ${this.formatSize(backupInfo.size)}`
        )
      );
  ...
  async restoreBackup(backupId, options = {}) {
    ...
    try {
      ...
      // Verify checksums
      for (const [relPath, expectedChecksum] of Object.entries(
        manifest.checksums
      )) {
      ...
      console.log(
        chalk.green(`✅ Restore ${dryRun ? 'preview' : 'completed'}`)
      );
      console.log(
        chalk.gray(`   Restored: ${result.restored_files.length} files`)
      );
      if (result.failed_files.length > 0) {
        console.log(
          chalk.red(`   Failed: ${result.failed_files.length} files`)
        );
      }
    } catch (error) {
      result.success = false;
    ...
    if (filter.metadata) {
      backups = backups.filter((b) => {
        return Object.entries(filter.metadata).every(
          ([key, value]) => b.metadata[key] === value
        );
      });
    }
