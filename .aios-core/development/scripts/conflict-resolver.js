...
class ConflictResolver {
  ...
  async detectConflicts() {
    try {
      ...
      return {
        ...
        totalConflicts: conflictDetails.reduce(
          (sum, f) => sum + f.conflictCount,
          0
        ),
      };
    } catch (error) {
  ...
  detectConflictType(file, conflictInfo) {
    ...
    for (const _conflict of conflicts) {
      ...
      if (
        oursContent.match(/\d+\.\d+\.\d+/) &&
        theirsContent.match(/\d+\.\d+\.\d+/)
      ) {
        return 'version';
      }
    }
  ...
  async resolveManual(_fileInfo) {
    ...
    console.log(
      chalk.yellow(`Manual resolution required for ${fileInfo.file}`)
    );
    console.log(
      chalk.gray('Edit the file to resolve conflicts, then mark as resolved')
    );
  ...
  async autoResolveVersion(_content, fileInfo) {
    ...
    for (const _conflict of fileInfo.conflicts) {
      ...
      const theirsVersion = conflict.theirs
        .join('')
        .match(/(\d+)\.(\d+)\.(\d+)/);
