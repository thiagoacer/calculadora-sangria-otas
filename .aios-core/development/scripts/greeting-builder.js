...
const {
  validateUserProfile,
} = require('../../infrastructure/scripts/validate-user-profile');
...
class GreetingBuilder {
  ...
  loadUserProfile(resolvedConfig) {
    try {
      ...
      if (!validation.valid) {
        console.warn(
          `[GreetingBuilder] user_profile validation failed: ${validation.error}`
        );
        return DEFAULT_USER_PROFILE;
      }
      if (validation.warning) {
        console.warn(
          `[GreetingBuilder] user_profile warning: ${validation.warning}`
        );
      }
      ...
      console.warn(
        '[GreetingBuilder] Failed to load user_profile:',
        error.message
      );
      return DEFAULT_USER_PROFILE;
    }
  ...
  async buildGreeting(agent, context = {}) {
    ...
    try {
      ...
      const greetingPromise = this._buildContextualGreeting(
        agent,
        context,
        userProfile
      );
      ...
      console.warn(
        '[GreetingBuilder] Fallback to simple greeting:',
        error.message
      );
  ...
  async _buildContextualGreeting(agent, context, preloadedUserProfile) {
    ...
    const sessionType =
      context.sessionType || (await this._safeDetectSessionType(context));

    const projectStatus =
      context.projectStatus || (await this._safeLoadProjectStatus());
    ...
    const permissionBadge =
      context.permissions?.badge || (await this._safeGetPermissionBadge());
    ...
    if (
      ...
      sections.push(
        this.buildProjectStatus(projectStatus, sessionType, sectionContext)
      );
    }
    ...
    const commands = this.filterCommandsByVisibility(
      agent,
      sessionType,
      userProfile
    );
    ...
    try {
      ...
      if (result && typeof result.then === 'function') {
        return await Promise.race([
          result,
          new Promise((resolve) =>
            setTimeout(() => resolve(null), SECTION_TIMEOUT)
          ),
        ]);
      }
      return result;
  ...
  buildFixedLevelGreeting(agent, level) {
    ...
    switch (level) {
      case 'minimal':
        greetingText =
          profile.greeting_levels.minimal ||
          `${agent.icon} ${agent.id} Agent ready`;
        break;
      case 'named':
        greetingText =
          profile.greeting_levels.named || `${agent.icon} ${agent.name} ready`;
        break;
      case 'archetypal':
        greetingText =
  ...
  buildSimpleGreeting(agent) {
    ...
    const greeting =
      greetingLevels?.named || `${agent.icon} ${agent.name} ready`;
    return `${greeting}\n\nType \`*help\` to see available commands.`;
  }
  ...
  buildPresentation(
    ...
    const greetingLevels =
      profile?.communication?.greeting_levels || profile?.greeting_levels;
    ...
    if (sessionType === 'existing' && sectionContext) {
      ...
      const namedGreeting =
        greetingLevels.named || `${agent.icon} ${agent.name} ready`;
      const storyRef =
        sectionContext.sessionStory ||
        sectionContext.projectStatus?.currentStory;
      if (storyRef) {
      ...
      const namedGreeting =
        greetingLevels.named || `${agent.icon} ${agent.name} ready`;
      const workflowPhase =
        sectionContext.workflowState?.currentPhase ||
        sectionContext.workflowActive;
      if (workflowPhase) {
  ...
  buildRoleDescription(agent, sectionContext = null) {
    ...
    if (sectionContext) {
      const storyRef =
        ...
        sectionContext.projectStatus?.currentStory;
      const branchRef =
        sectionContext.projectStatus?.branch ||
        sectionContext.gitConfig?.branch;
  ...
  buildContextSection(
    ...
    const contextNarrative = this._buildContextNarrative(
      agent,
      context,
      projectStatus
    );
    ...
    if (
      sectionContext &&
      sectionContext.previousAgent &&
      !contextNarrative.description
    ) {
    ...
    if (contextNarrative.recommendedCommand) {
      parts.push(
        `   **Recommended:** Use \`${contextNarrative.recommendedCommand}\` to continue`
      );
    }
  ...
  _buildContextNarrative(agent, context, projectStatus) {
    ...
    if (prevAgentId && projectStatus?.modifiedFiles) {
      ...
      const fileContext = this._analyzeModifiedFiles(
        projectStatus.modifiedFiles,
        sessionStory
      );
      ...
      const recommendedCommand = this._suggestCommand(
        agent.id,
        prevAgentId,
        storyContext
      );
  ...
  _analyzeStoryContext(projectStatus) {
    ...
    return {
      ...
      storyFile: storyFile,
      hasStory:
        !!currentStory && currentStory !== 'EPIC-SPLIT-IMPLEMENTATION-COMPLETE',
    };
  }
  ...
  _analyzeModifiedFiles(modifiedFiles, _currentStory) {
    ...
    const patterns = [
      ...
      },
      {
        regex: /\.md$/,
        priority: 2,
        desc: 'dos arquivos de documentação',
        category: 'doc',
      },
    ];
  ...
  _suggestCommand(agentId, prevAgentId, storyContext) {
    ...
    if (prevAgentId === 'dev' && agentId === 'qa') {
      return storyContext.storyFile
        ? `*review ${storyContext.storyFile}`
        : '*review';
    }
  ...
  buildWorkflowSuggestions(context) {
    try {
      ...
      const commandHistory =
        context.commandHistory || context.lastCommands || [];
      const workflowState = this.workflowNavigator.detectWorkflowState(
        commandHistory,
        context
      );
      ...
      const suggestions =
        this.workflowNavigator.suggestNextCommands(workflowState);
      if (!suggestions || suggestions.length === 0) {
        return null;
      }

      // Story ACT-5 (AC: 4): Enhance suggestions with SurfaceChecker proactive triggers
      const enhancedSuggestions = this._enhanceSuggestionsWithSurface(
        suggestions,
        context
      );

      const greetingMessage =
        this.workflowNavigator.getGreetingMessage(workflowState);
      const header = greetingMessage || 'Next steps:';

      return this.workflowNavigator.formatSuggestions(
        enhancedSuggestions,
        header
      );
    } catch (error) {
  ...
  _detectWorkflowFromSessionState() {
    try {
      ...
      console.warn(
        '[GreetingBuilder] SessionState workflow detection failed:',
        error.message
      );
      return null;
    }
  ...
  _enhanceSuggestionsWithSurface(suggestions, context) {
    try {
      ...
      console.warn(
        '[GreetingBuilder] SurfaceChecker enhancement failed:',
        error.message
      );
      return suggestions;
    }
  ...
  buildContextualSuggestions(agent, projectStatus, _sessionType) {
    try {
      ...
      if (projectStatus.currentStory) {
        const storyMatch = projectStatus.currentStory.match(
          /(\d+\.\d+\.\d+(\.\d+)?)/
        );
        ...
        if (
          ...
          if (
            recentCommit.includes('complete') ||
            recentCommit.includes('implement')
          ) {
            if (storyId) {
      ...
      if (
        ...
        if (
          (lastCommit.includes('feat:') || lastCommit.includes('feature')) &&
          agentId === 'qa'
        ) {
          suggestions.push('*code-review committed');
        }
      ...
      console.warn(
        '[GreetingBuilder] Contextual suggestions failed:',
        error.message
      );
      return null;
    }
  ...
  _buildContextSummary(projectStatus) {
    ...
    if (projectStatus.recentCommits && projectStatus.recentCommits.length > 0) {
      const lastCommit = projectStatus.recentCommits[0].message;
      const shortMsg =
        lastCommit.length > 50
          ? lastCommit.substring(0, 47) + '...'
          : lastCommit;
...
**Para interagir com Bob:**
  ...
  buildFooter(agent, sectionContext = null) {
    ...
    if (sessionType === 'workflow') {
      ...
      if (storyRef) {
        parts.push(
          `Focused on **${storyRef}**. Type \`*help\` to see available commands.`
        );
      } else {
      ...
      parts.push(
        'Type `*help` for commands or `*session-info` for session details.'
      );
    } else {
  ...
  async _safeGetPermissionBadge() {
    try {
      ...
      console.warn(
        '[GreetingBuilder] Permission mode load failed:',
        error.message
      );
      return '';
    }
