    ...
    switch (command) {
      case 'get':
        if (subcommand === 'greeting') {
          ...
          console.log(
            `Preference: ${config.preference || 'auto'} (auto|minimal|named|archetypal)`
          );
          console.log(
            `Context Detection: ${config.contextDetection !== false ? 'enabled' : 'disabled'}`
          );
          console.log(
            `Session Detection: ${config.sessionDetection || 'hybrid'}`
          );
          console.log(
            `Workflow Detection: ${config.workflowDetection || 'hardcoded'}`
          );
          console.log(
            `Show Archetype: ${config.showArchetype !== false ? 'yes' : 'no'}`
          );
      ...
      case 'set':
        if (subcommand === 'greeting.preference' && value) {
          ...
          if (value === 'auto') {
            ...
            console.log(
              '  - Workflow session → Minimal greeting + suggestions'
            );
          } else {
