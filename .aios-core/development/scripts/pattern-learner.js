...
class PatternLearner extends EventEmitter {
  ...
  async recordSuccessfulModification(modification) {
    ...
    console.log(
      chalk.green(`✅ Recorded successful modification: ${record.id}`)
    );
  ...
  async extractPatterns(modification) {
    ...
    if (modification.structuralChanges) {
      patterns.push(
        ...this.extractStructuralPatterns(modification.structuralChanges)
      );
    }
    ...
    if (modification.dependencyChanges) {
      patterns.push(
        ...this.extractDependencyPatterns(modification.dependencyChanges)
      );
    }
    ...
    if (modification.performanceImprovements) {
      patterns.push(
        ...this.extractPerformancePatterns(modification.performanceImprovements)
      );
    }
  ...
  async calculatePatternSimilarity(pattern1, pattern2) {
    ...
    switch (pattern1.type) {
      case 'code_transformation':
        ...
        similarity +=
          this.calculateContextSimilarity(pattern1.context, pattern2.context) *
          0.3;
        factors = 1;
        break;
      ...
      case 'structural':
        ...
        similarity +=
          this.calculateBenefitSimilarity(
            pattern1.benefits,
            pattern2.benefits
          ) * 0.4;
        factors = 1;
  ...
  async updatePattern(similarPattern, newPattern, record) {
    ...
    existingPattern.successRate =
      existingPattern.successCount / existingPattern.occurrences;
    ...
    existingPattern.confidence =
      this.calculatePatternConfidence(existingPattern);

    // Check if pattern should be promoted
    if (
      existingPattern.occurrences >= this.learningThreshold &&
      ...
      console.log(
        chalk.green(`✅ Pattern promoted to learned: ${similarPattern.key}`)
      );
  ...
  isPatternApplicable(pattern, context) {
    ...
    if (pattern.componentType && context.componentType) {
      if (
        pattern.componentType !== context.componentType &&
  ...
  async generateApplicationGuide(pattern, context) {
    ...
    switch (pattern.type) {
      ...
      case 'refactoring':
        ...
        break;
      case 'performance':
        guide.steps = this.generatePerformanceOptimizationSteps(
          pattern,
          context
        );
  ...
  async calculateEffectivenessMetrics() {
    ...
    if (learnedPatterns.length > 0) {
      metrics.averageSuccessRate =
        learnedPatterns.reduce((sum, p) => sum + p.successRate, 0) /
        learnedPatterns.length;

      metrics.averageConfidence =
        learnedPatterns.reduce((sum, p) => sum + p.confidence, 0) /
        learnedPatterns.length;
    }

    // Calculate pattern coverage
    const modificationTypes = new Set(
      this.modificationHistory.map((m) => m.modificationType)
    );
    const coveredTypes = new Set(learnedPatterns.map((p) => p.type));
    metrics.patternCoverage =
      modificationTypes.size > 0
        ? coveredTypes.size / modificationTypes.size
        : 0;
    ...
    const recentLearned = recentHistory.filter((m) =>
      m.patterns.some(
        (p) =>
          this.patterns.get(this.generatePatternKey(p))?.status === 'learned'
      )
    );
    metrics.learningRate =
      recentHistory.length > 0
        ? recentLearned.length / recentHistory.length
        : 0;
  ...
  aggregateBenefits(existing, newBenefits) {
    ...
    for (const [key, value] of Object.entries(newBenefits)) {
      if (typeof value === 'number') {
        ...
        aggregated[`${key}_avg`] =
          aggregated[key] / ((aggregated[`${key}_count`] || 0) + 1);
  ...
  calculateConsistencyFactor(usageHistory) {
    ...
    for (let i = 1; i < usageHistory.length; i++) {
      const interval =
        new Date(usageHistory[i].timestamp) -
        new Date(usageHistory[i - 1].timestamp);
  ...
  calculateRelevance(pattern, context) {
    ...
    if (pattern.benefits && context.goals) {
      relevance +=
        this.calculateGoalAlignment(pattern.benefits, context.goals) * 0.2;
    }
    ...
    if (refactor1.triggers && refactor2.triggers) {
      ...
      similarity +=
        (commonTriggers.length /
          Math.max(refactor1.triggers.length, refactor2.triggers.length)) *
        0.3;
    }
    ...
    if (perf1.applicableContexts && perf2.applicableContexts) {
      ...
      similarity +=
        (commonContexts.length /
          Math.max(
            perf1.applicableContexts.length,
            perf2.applicableContexts.length
          )) *
        0.3;
    }
  ...
  generatePerformanceOptimizationSteps(pattern, context) {
    ...
    steps.push({
      ...
      description:
        pattern.description || 'Apply performance optimization technique',
