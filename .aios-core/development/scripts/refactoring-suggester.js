...
class RefactoringSuggester {
        ...
        try {
          ...
          if (detected && detected.length > 0) {
            for (const _detection of detected) {
              const suggestion = await pattern.suggester(
                _detection,
                _ast,
                content
              );
          ...
          console.warn(
            chalk.yellow(`Failed to run ${pattern.name}: ${error.message}`)
          );
        }
      }
  ...
  parseCode(_content, filePath) {
    ...
    try {
      ...
      console.warn(
        chalk.yellow(`Parse error in ${filePath}: ${error.message}`)
      );
  ...
  async detectMagicNumbers(_ast, _content, metrics) {
    ...
    traverse(_ast, {
      NumericLiteral: (path) => {
        ...
        if (
          path.findParent(
            (p) => p.isVariableDeclarator() && p.parent.kind === 'const'
          )
        )
          return;
  ...
  async detectLargeClasses(_ast, _content, metrics) {
    ...
    traverse(_ast, {
      ClassDeclaration: (path) => {
        ...
        const methodCount = methods.filter(
          (m) => m.type === 'ClassMethod'
        ).length;
        const propertyCount = methods.filter(
          (m) => m.type === 'ClassProperty'
        ).length;

        if (
          methodCount > methodThreshold ||
          propertyCount > propertyThreshold
        ) {
          largeClasses.push({
            type: 'large_class',
            node: path.node,
  ...
  async suggestMethodExtraction(_detection, _ast, content) {
    const suggestion = {
      ...
      impact: Math.min(
        10,
        Math.floor(detection.lines / 10) + Math.floor(detection.complexity / 5)
      ),
  ...
  async suggestVariableExtraction(_detection, _ast, content) {
    const suggestion = {
      ...
      suggestedRefactoring:
        this.generateVariableExtractionSuggestion(_detection),
    };
  ...
  async suggestDeadCodeRemoval(_detection, _ast, content) {
    const suggestion = {
      ...
      suggestedRefactoring: {
        ...
        lines: [
          detection.startLine,
          detection.node.loc?.end.line || detection.startLine,
        ],
      },
    };
  ...
  async suggestConditionalSimplification(_detection, _ast, content) {
    const suggestion = {
      ...
      suggestedRefactoring:
        this.generateConditionalSimplificationSuggestion(_detection),
    };
  ...
  async suggestConstantExtraction(_detection, _ast, content) {
    const suggestion = {
      ...
      suggestedRefactoring:
        this.generateConstantExtractionSuggestion(_detection),
    };
  ...
  async suggestClassDecomposition(_detection, _ast, content) {
    const suggestion = {
      ...
      suggestedRefactoring:
        this.generateClassDecompositionSuggestion(_detection),
    };
  ...
  calculateExpressionComplexity(node, depth = 0) {
    ...
    if (node.type === 'CallExpression') {
      maxDepth = Math.max(
        maxDepth,
        this.calculateExpressionComplexity(node.callee, depth + 1)
      );
      for (const arg of node.arguments) {
        maxDepth = Math.max(
          maxDepth,
          this.calculateExpressionComplexity(arg, depth + 1)
        );
      }
    } else if (node.type === 'MemberExpression') {
      maxDepth = Math.max(
        maxDepth,
        this.calculateExpressionComplexity(node.object, depth + 1)
      );
  ...
  async applySuggestion(suggestion, options = {}) {
    ...
    try {
      ...
      switch (suggestion.type) {
        ...
        default:
          throw new Error(
            `Refactoring type ${suggestion.type} not implemented`
          );
      }
  ...
  getStatistics() {
    ...
    stats.averageImpact =
      stats.totalSuggestions > 0
        ? (totalImpact / stats.totalSuggestions).toFixed(2)
        : 0;
