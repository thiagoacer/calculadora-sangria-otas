...
class RollbackHandler {
  ...
  async undoLast(options = {}) {
    try {
      ...
      if (options.transactionId) {
        ...
        transaction = await this.transactionManager.loadTransaction(
      ...
      console.log(
        chalk.gray(`Date: ${new Date(transaction.startTime).toLocaleString()}`)
      );
      ...
      for (const op of transaction.operations) {
        ...
        console.log(
          chalk.gray(`  ${icon} ${op.type}: ${path.basename(op.path)}`)
        );
      }
  ...
  async listTransactions(limit = 10) {
    try {
      const transactions =
        await this.transactionManager.listTransactions(limit);
  ...
  async selectiveRollback(transactionId, operationIds) {
    try {
      const transaction =
        await this.transactionManager.loadTransaction(transactionId);
      if (!transaction) {
  ...
  async restoreFromBackup(backupPath, targetPath) {
    try {
      ...
      console.log(
        chalk.green(`✅ Restored from backup: ${path.basename(backupPath)}`)
      );
      return true;
    } catch (error) {
      console.error(
        chalk.red(`Failed to restore from backup: ${error.message}`)
      );
      return false;
    }
  }
  ...
  async rollbackModification(modificationData) {
    const { componentType, componentName, backupPath, targetPath } =
      modificationData;
  ...
  async listBackups(componentType, componentName) {
    try {
      ...
      const backups = files
        .filter(
          (file) =>
            file.startsWith(`${componentName}.`) && file.endsWith('.backup')
        )
        .map((file) => {
  ...
  displayRollbackResults(results) {
    ...
    if (results.successful.length > 0) {
      ...
      results.successful.forEach((item) => {
        console.log(
          chalk.green(`   ✓ ${item.action}: ${path.basename(item.path)}`)
        );
      });
    }
    ...
    if (results.warnings.length > 0) {
      ...
      results.warnings.forEach((item) => {
        console.log(
          chalk.yellow(
            `   ⚠ ${item.warning}: ${path.basename(item.path || 'N/A')}`
          )
        );
      });
    }
    ...
    const successRate =
      total > 0 ? ((results.successful.length / total) * 100).toFixed(0) : 0;
