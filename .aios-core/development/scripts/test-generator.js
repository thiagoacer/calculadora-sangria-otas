...
class TestGenerator {
  ...
  async initialize() {
    try {
      ...
      console.error(
        chalk.red(`Failed to initialize test generator: ${error.message}`)
      );
      throw error;
    }
  }
  ...
  async generateTestFile(component, testFile, config) {
    ...
    try {
      console.log(
        chalk.blue(
          `🧪 Generating ${testFile.test_type} test for ${component.name}`
        )
      );
      ...
      const processedContent = await this.postProcessTestContent(
        testContent,
        component,
        config
      );
      ...
      console.log(
        chalk.green(
          `✅ Generated ${testFile.test_type} test for ${component.name}`
        )
      );
      ...
      console.error(
        chalk.red(
          `Failed to generate test for ${component.name}: ${error.message}`
        )
      );
      throw error;
    }
  ...
  async generateTestSuite(component, testSuite, config) {
    ...
    for (const testFile of testSuite.test_files) {
      try {
        const testContent = await this.generateTestFile(
          component,
          testFile,
          config
        );
    ...
    console.log(
      chalk.gray(
        `   Generated: ${generatedFiles.length}/${testSuite.test_files.length} files`
      )
    );
    console.log(
      chalk.gray(`   Success rate: ${Math.round(result.success_rate * 100)}%`)
    );
  ...
  async generateTestContent(component, testFile, config) {
    ...
    const baseContent = await this.templateSystem.generateTestContent(
      component,
      testFile,
      config
    );
    ...
    const enhancedContent = await this.enhanceTestContent(
      baseContent,
      component,
      testFile,
      config
    ...
    const optimizedContent = await this.optimizeForFramework(
      enhancedContent,
      config.framework
    );
  ...
  async enhanceTestContent(baseContent, component, testFile, config) {
    ...
    try {
      ...
      if (additionalTestCases.length > 0) {
        enhancedContent = this.injectAdditionalTestCases(
          enhancedContent,
          additionalTestCases
        );
      }
      ...
      const dynamicImports = await this.generateDynamicImports(
        componentAnalysis,
        config
      );
      if (dynamicImports) {
        enhancedContent = this.injectDynamicImports(
          enhancedContent,
          dynamicImports
        );
      }
      ...
      const setupTeardown = await this.generateSetupTeardown(
        componentAnalysis,
        testFile.test_type
      );
      if (setupTeardown) {
        enhancedContent = this.injectSetupTeardown(
          enhancedContent,
          setupTeardown
        );
      }
      ...
      console.warn(
        chalk.yellow(`Failed to enhance test content: ${error.message}`)
      );
  ...
  async analyzeComponent(component) {
    ...
    try {
      ...
      console.warn(
        chalk.yellow(
          `Failed to analyze component ${component.id}: ${error.message}`
        )
      );
    }
  ...
  async generateAdditionalTestCases(componentAnalysis, _testType, config) {
    ...
    for (const func of componentAnalysis.functions) {
      if (func.visibility === 'public') {
        additionalCases.push(
          ...this.generateFunctionTestCases(func, _testType, config)
        );
      }
    }
    ...
    for (const cls of componentAnalysis.classes) {
      additionalCases.push(
        ...this.generateClassTestCases(cls, _testType, config)
      );
    }
    ...
    if (componentAnalysis.error_handling) {
      additionalCases.push(
        ...this.generateErrorHandlingTestCases(_testType, config)
      );
    }
  ...
  optimizeForJest(content) {
    ...
    if (
      content.includes('toBeInstanceOf') &&
      !content.includes('expect.extend')
    ) {
...
${content}`;
  ...
  async validateTestSyntax(content, framework) {
    try {
      ...
      console.warn(
        chalk.yellow(`Test syntax validation warning: ${error.message}`)
      );
    }
  }
  ...
  injectAdditionalTestCases(content, testCases) {
    ...
    const additionalTests = testCases
      .map((testCase) => {
        ...
        caseContent += testCase.assertions
  ...
  injectDynamicImports(content, imports) {
    ...
    if (existingImports && existingImports.length > 0) {
      ...
      const insertionPoint =
        lastImportIndex + existingImports[existingImports.length - 1].length;
      return (
        content.slice(0, insertionPoint) +
        '\n' +
        importSection +
  ...
  extractClasses(content) {
    ...
    const classDeclarations = content.match(
  ...
  generateDynamicImports(componentAnalysis, config) {
    ...
    for (const dep of componentAnalysis.dependencies) {
      if (dep.type === 'local') {
        imports.push(
          `const ${this.toVariableName(dep.name)} = require('${dep.name}');`
        );
      }
    }
  ...
  generateSetupTeardown(componentAnalysis, testType) {
    ...
    if (
      componentAnalysis.type === 'util' &&
      componentAnalysis.classes.length > 0
    ) {
  ...
  getGenerationStats() {
    return {
      ...
      average_generation_time:
        this.generationStats.total_generated > 0
          ? this.generationStats.generation_time /
            this.generationStats.total_generated
          : 0,
    };
  }
