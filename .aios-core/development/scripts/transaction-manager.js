...
class TransactionManager {
  constructor(options = {}) {
    ...
    this.transactionPath = path.join(
      this.rootPath,
      'aios-core',
      'transactions'
    );
  ...
  async recordOperation(transactionId, operation) {
    try {
      ...
      if (operation.type === 'update' || operation.type === 'delete') {
        operationRecord.previousState = await this.backupCurrentState(
          operation.path
        );
      }
  ...
  async commitTransaction(transactionId) {
    try {
      ...
      transaction.duration =
        new Date(transaction.endTime) - new Date(transaction.startTime);
      ...
      console.error(
        chalk.red(`Failed to commit transaction: ${error.message}`)
      );
      throw error;
    }
  }
  ...
  async rollbackTransaction(transactionId, options = {}) {
    try {
      ...
      console.log(
        chalk.yellow(`⚙️  Rolling back transaction: ${transactionId}`)
      );
      ...
      console.log(
        chalk.gray(`   Successful: ${rollbackResults.successful.length}`)
      );
      console.log(chalk.gray(`   Failed: ${rollbackResults.failed.length}`));
      console.log(
        chalk.gray(`   Warnings: ${rollbackResults.warnings.length}`)
      );
  ...
  async rollbackOperation(operation, results) {
    console.log(
      chalk.gray(`  Rolling back: ${operation.type} ${operation.path}`)
    );
  ...
  async getLastTransaction() {
    try {
      ...
      console.error(
        chalk.red(`Failed to get last transaction: ${error.message}`)
      );
      return null;
    }
  }
  ...
  async rollbackMetadataUpdate(operation, results) {
    try {
      if (
        operation.metadata?.componentType &&
        operation.metadata?.componentId
      ) {
