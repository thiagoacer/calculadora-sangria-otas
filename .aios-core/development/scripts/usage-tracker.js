...
class UsageTracker {
  ...
  async initialize() {
    try {
      ...
      console.error(
        chalk.red(`Failed to initialize usage tracker: ${error.message}`)
      );
      throw error;
    }
  }
  ...
  async analyzeComponentUsage(componentId, options = {}) {
    ...
    try {
      console.log(
        chalk.blue(`🔍 Analyzing usage for component: ${componentId}`)
      );
      ...
      const directReferences = await this.scanForDirectReferences(
        component,
        options
      );
      ...
      const dependencies = await this.analyzeDependencyRelationships(
        component,
        options
      );
      ...
      const externalRefs = await this.scanForExternalReferences(
        component,
        options
      );
      ...
      analysis.usage_patterns = await this.analyzeUsagePatterns(
        component,
        directReferences
      );
      ...
      console.log(
        chalk.green(`✅ Usage analysis completed for ${componentId}`)
      );
      console.log(
        chalk.gray(`   Total references: ${analysis.total_references}`)
      );
      console.log(
        chalk.gray(
          `   Dependent components: ${analysis.dependent_components.length}`
        )
      );
      console.log(
        chalk.gray(`   Impact level: ${analysis.risk_assessment.impact_level}`)
      );

      return analysis;
    } catch (_error) {
      console.error(
        chalk.red(`Usage analysis failed for ${componentId}: ${error.message}`)
      );
      throw error;
    }
  }
  ...
  async getUsageStatistics(componentIds = null) {
    ...
    const componentsToAnalyze =
      componentIds || (await this.getAllTrackedComponents());
    ...
    for (const componentId of componentsToAnalyze) {
      try {
        ...
        stats.usage_distribution[usageRange] =
          (stats.usage_distribution[usageRange] || 0) + 1;
        ...
        console.warn(
          chalk.yellow(
            `Failed to analyze usage for ${componentId}: ${error.message}`
          )
        );
      }
    }
  ...
  async scanForDirectReferences(component, options = {}) {
    ...
    for (const scanPath of scanPaths) {
      try {
        ...
        console.warn(
          chalk.yellow(`Failed to scan path ${scanPath}: ${error.message}`)
        );
      }
    }
  ...
  findReferencesInLine(line, component) {
    const matches = [];
    const patterns =
      this.scanPatterns[component.type] || this.scanPatterns.default;
  ...
  async scanConfigurationFiles(component) {
    ...
    for (const configFile of configFiles) {
      ...
      try {
        ...
        configRefs.push(
          ...refs.map((ref) => ({ ...ref, source: 'configuration' }))
        );
      } catch (_error) {
  ...
  async scanDocumentationFiles(component) {
    ...
    try {
      ...
      for (const file of files) {
        ...
        docRefs.push(
          ...refs.map((ref) => ({ ...ref, source: 'documentation' }))
        );
      }
    } catch (_error) {
  ...
  calculateUsageTrend(baseline, current) {
    const referenceChange =
      current.total_references - baseline.total_references;
