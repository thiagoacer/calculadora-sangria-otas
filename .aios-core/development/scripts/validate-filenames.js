...
class FilenameValidator {
  ...
  validateFilename(filename, directory) {
    ...
    for (const [ptTerm, enTerm] of Object.entries(PORTUGUESE_TERMS)) {
      if (
        lowerBasename.includes(ptTerm) &&
        !STANDARD_FILES.includes(filename)
      ) {
        const suggested = lowerBasename.replace(
          new RegExp(ptTerm, 'g'),
          enTerm
        );
        this.errors.push({
  ...
  report() {
    ...
    if (this.errors.length > 0) {
      ...
      console.log(
        '2. Or manually rename the files using the suggestions above'
      );
      console.log(
        '3. Or run: node .aios-core/utils/validate-filenames.js --fix\n'
      );
