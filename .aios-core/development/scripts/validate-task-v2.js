...
const validationRules = [
  ...
  },
  {
    ...
    check: (content) => {
      ...
      const hasValidation =
        content.includes('validação:') || content.includes('validation:');
      return hasType && hasBlocker && hasValidation;
    },
    message:
      'Checklist items missing required fields (tipo, blocker, validação)',
  },
  {
    id: 6,
  ...
  },
  {
    ...
    message:
      'Missing Performance section or required metrics (duration_expected, cost_estimated)',
  },
  {
    id: 9,
...
function validateAllTasks() {
  ...
  if (!fs.existsSync(tasksDir)) {
    console.error(
      `${colors.red}✗ Tasks directory not found: ${tasksDir}${colors.reset}`
    );
    process.exit(2);
  }
  ...
  console.log(
    `${colors.cyan}Validating ${taskFiles.length} tasks...${colors.reset}\n`
  );
...
function printResult(result) {
  ...
  if (result.compliant) {
    ...
    result.failed.forEach((failure) => {
      console.log(
        `   ${colors.red}✗${colors.reset} Rule ${failure.id}: ${failure.message}`
      );
    });
...
function main() {
  ...
  if (args[0] === '--all') {
    ...
    if (!fs.existsSync(taskFile)) {
      console.error(
        `${colors.red}✗ File not found: ${taskFile}${colors.reset}`
      );
      process.exit(2);
    }
