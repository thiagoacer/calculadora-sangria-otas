...
function verifyGap1() {
  ...
  const createWf = fs.readFileSync(
    path.join(ROOT, '.aios-core/development/tasks/create-workflow.md'),
    'utf-8'
  );
  assert(
  ...
  const modifyWf = fs.readFileSync(
    path.join(ROOT, '.aios-core/development/tasks/modify-workflow.md'),
    'utf-8'
  );
  assert(
  ...
  const targetContextQ = firstStep.questions.find(
    (q) => q.name === 'targetContext'
  );
  assert(
    !!targetContextQ,
  ...
  const template = fs.readFileSync(
    path.join(ROOT, '.aios-core/product/templates/workflow-template.yaml'),
    'utf-8'
  );
  assert(
...
async function verifyGap2() {
  ...
  try {
    ({
      WorkflowValidator,
    } = require('../../../.aios-core/development/scripts/workflow-validator'));
  ...
  const goodResult = await validator.validate(
    path.join(ROOT, '.aios-core/development/workflows/greenfield-service.yaml')
  );
  assert(
  ...
  const formatted = validator.formatResult(
    goodResult,
  ...
  const {
    SquadValidator,
  } = require('../../../.aios-core/development/scripts/squad/squad-validator');
  ...
  const faResult = await fa.validateWorkflow({
    id: 'test',
    name: 'Test',
    sequence: [],
  });
  ...
  assert(
    fs.existsSync(
      path.join(ROOT, '.aios-core/development/tasks/validate-workflow.md')
    ),
  ...
  const masterMd = fs.readFileSync(
    path.join(ROOT, '.aios-core/development/agents/aios-master.md'),
    'utf-8'
  );
  assert(
...
async function verifyGap3() {
  ...
  try {
    ...
    const wfContent = fs.readFileSync(
      path.join(
        ROOT,
        '.aios-core/development/workflows/greenfield-service.yaml'
      ),
      'utf-8'
    );
    ...
    const optionalIdx = state.steps.findIndex(
      (s) => s.optional && s.status === 'pending'
    );
    if (optionalIdx !== -1) {
    ...
    const requiredIdx = state.steps.findIndex(
      (s) => !s.optional && s.status === 'pending'
    );
    if (requiredIdx !== -1) {
      let threw = false;
    ...
    const patterns = yaml.load(
      fs.readFileSync(
        path.join(ROOT, '.aios-core/data/workflow-patterns.yaml'),
        'utf-8'
      )
    );
    assert(
    ...
    const schemaContent = fs.readFileSync(
      path.join(ROOT, '.aios-core/data/workflow-state-schema.yaml'),
      'utf-8'
    );
    ...
    assert(
      fs.existsSync(
        path.join(ROOT, '.aios-core/development/tasks/run-workflow.md')
      ),
...
async function verifyGap4() {
  ...
  const hybridChoice =
    targetContextQ && targetContextQ.choices.find((c) => c.value === 'hybrid');
  assert(
    !!hybridChoice,
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(tmpCoreAgents, 'architect.md'),
      '# Architect Agent'
    );
    fs.writeFileSync(
      path.join(tmpSquadAgents, 'validator.md'),
      '# Validator Agent'
    );
    ...
    const missingWarning = missingResult.warnings.some(
      (w) =>
        w.code === 'WF_AGENT_NOT_FOUND' &&
        w.message.includes('nonexistent-agent')
    );
    assert(
      missingWarning,
      '4.7 Validator warns for agent not found in either context',
    ...
    for (const taskFile of taskFiles) {
      const content = fs.readFileSync(
        path.join(ROOT, '.aios-core/development/tasks', taskFile),
        'utf-8'
      );
      assert(
    ...
    if (hybridStateMgr && hybridState) {
      const cleanupPath = hybridStateMgr._resolveStatePath(
        hybridState.instance_id
      );
      try {
