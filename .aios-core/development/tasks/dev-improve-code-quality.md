...
class ImproveCodeQualityTask {
  ...
  async loadCustomConfig(config) {
    try {
      ...
      console.warn(
        chalk.yellow(`Failed to load custom config: ${error.message}`)
      );
    }
  }
  ...
  async initializeDependencies() {
    try {
      ...
      this.codeQualityImprover = new CodeQualityImprover({
  ...
  async previewImprovements(config) {
    ...
    console.log(chalk.blue('\n📌 Next Steps:'));
    console.log(
      `1. Apply all improvements: *improve-code-quality ${config.targetPath} --auto-fix`
    );
    console.log(
      `2. Interactive selection: *improve-code-quality ${config.targetPath}`
    );
    console.log(
  ...
  async interactiveImprovement(config) {
    ...
    for (const [file, improvements] of Object.entries(byFile)) {
      ...
      if (selected.length > 0) {
        const selectedImprovements = improvements.filter((imp) =>
          selected.includes(imp.id)
        );
        ...
        try {
          ...
          if (result.success) {
            console.log(
              chalk.green(`✅ Applied ${result.appliedCount} improvements`)
            );
  ...
  displayStatistics() {
    ...
    const sortedPatterns = Object.entries(patternStats).sort(
      ([, a], [, b]) => b.total - a.total
    );
