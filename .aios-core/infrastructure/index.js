...
function safeRequire(modulePath) {
  try {
    ...
    if (process.env.AIOS_DEBUG) {
      console.warn(
        `[infrastructure] Failed to load ${modulePath}: ${error.message}`
      );
    }
    return null;
  }
...
const CommitMessageGenerator = safeRequire(
  './scripts/commit-message-generator'
);
...
const validateOutputPatternModule = safeRequire(
  './scripts/validate-output-pattern'
);
...
const DependencyImpactAnalyzer = safeRequire(
  './scripts/dependency-impact-analyzer'
);
...
const ModificationRiskAssessment = safeRequire(
  './scripts/modification-risk-assessment'
);
...
const PerformanceAndErrorResolver = safeRequire(
  './scripts/performance-and-error-resolver'
);
...
const DocumentationSynchronizer = safeRequire(
  './scripts/documentation-synchronizer'
);
...
const loadProjectStatus = projectStatusLoader
  ? projectStatusLoader.loadProjectStatus
  : null;
const formatStatusDisplay = projectStatusLoader
  ? projectStatusLoader.formatStatusDisplay
  : null;
