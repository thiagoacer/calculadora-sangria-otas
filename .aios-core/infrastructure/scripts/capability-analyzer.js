...
class CapabilityAnalyzer {
  ...
  async analyzeCapabilities(options = {}) {
    const {
      target_areas = Object.keys(this.categories),
    ...
    for (const category of target_areas) {
      ...
      const categoryAnalysis = await this.analyzeCategory(
        category,
        currentImplementation
      );
      ...
      analysis.improvement_opportunities.push(
        ...categoryAnalysis.opportunities
      );
    }
  ...
  async analyzeMetric(category, metric, basePath) {
    const methodName = `analyze_${category}_${metric}`.replace(
      /_([a-z])/g,
      (g) => g[1].toUpperCase()
    );
  ...
  async analyzeErrorHandlingTryCatchCoverage(basePath) {
    ...
    const coverage =
      totalFunctions > 0 ? (functionsWithTryCatch / totalFunctions) * 100 : 0;
  ...
  async analyzeErrorHandlingErrorContext(basePath) {
    ...
    for (const file of files) {
      try {
        const content = await fs.readFile(file, 'utf-8');
        const errorMatches =
          content.match(/throw\s+new\s+Error\([^)]+\)/g) || [];
    ...
    const contextRate =
      totalErrors > 0 ? (contextualErrors / totalErrors) * 100 : 100;
  ...
  async analyzeErrorHandlingRetryLogic(basePath) {
    ...
    for (const file of files) {
      try {
        ...
        const networkOps =
          content.match(/fetch|axios|request|http\.|https\./g) || [];
  ...
  async generateChange(opportunity, analysis) {
    ...
    switch (`${opportunity.category}_${opportunity.metric}`) {
      case 'error_handling_try_catch_coverage':
        const details =
          analysis.categories.error_handling.metrics.try_catch_coverage.details;
      ...
      case 'error_handling_error_context':
        const errorDetails =
          analysis.categories.error_handling.metrics.error_context.details;
