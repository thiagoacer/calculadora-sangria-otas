...
class ComponentGenerator {
  ...
  async generateComponent(componentType, options = {}) {
    ...
    try {
      ...
      const elicitationSteps =
        await this.loadElicitationWorkflow(componentType);
      ...
      const answers =
        await this.elicitationEngine.runProgressive(elicitationSteps);
      if (!answers) {
        if (!options.transactionId) {
      ...
      const previewMetadata = {
        name:
          variables.AGENT_NAME || variables.TASK_ID || variables.WORKFLOW_ID,
        path: outputPath,
        type: componentType,
      };
      ...
      const stats = this.componentPreview.getPreviewStats(
        content,
        componentType
      );
      console.log(chalk.gray('\n📊 Component Statistics:'));
      console.log(
        chalk.gray(`   Lines: ${stats.lines} | Size: ${stats.size} bytes`)
      );
      if (stats.commands !== undefined) {
        console.log(
          chalk.gray(
            `   Commands: ${stats.commands} | Dependencies: ${stats.dependencies}`
          )
        );
      }
      if (stats.steps !== undefined) {
        console.log(
          chalk.gray(`   Steps: ${stats.steps} | Triggers: ${stats.triggers}`)
        );
      }
      ...
      const proceed =
        options.skipPreview ||
        (await this.componentPreview.confirmWithPreview(preview, {
      ...
      if (!proceed) {
        ...
        return {
          success: false,
          error: 'Component creation cancelled by user',
        };
      }
      ...
      if (await fs.pathExists(outputPath)) {
        const overwrite =
          options.force || (await this.confirmOverwrite(outputPath));
        if (!overwrite) {
      ...
      if (componentType === 'agent' && !options.skipTaskCreation) {
        await this.createAgentCommandTasks(variables, {
          ...options,
          transactionId,
        });
      }
  ...
  processAnswersToVariables(componentType, answers) {
    ...
    switch (componentType) {
      case 'agent':
        ...
        variables.PERSONA_FOCUS =
          answers.personaFocusCustom || answers.personaFocus;

        // Commands
        const commands = [];
        if (answers.standardCommands && answers.standardCommands.length > 0) {
          commands.push(
            ...answers.standardCommands.map((cmd) => ({
              COMMAND_NAME: cmd,
        ...
        if (answers.customCommands && Array.isArray(answers.customCommands)) {
          commands.push(
            ...answers.customCommands.map((cmd) => {
        ...
        if (answers.dependencyTypes && answers.dependencyTypes.length > 0) {
          ...
          if (
            answers.templateDependencies &&
            answers.templateDependencies.length > 0
          ) {
        ...
        if (answers.securityLevel !== 'standard') {
          ...
          variables.SECURITY_AUTHORIZATION = answers.requireAuthorization
            ? 'required'
            : 'optional';
        ...
        if (answers.customActivationInstructions) {
          ...
          variables.ACTIVATION_INSTRUCTIONS =
            answers.customActivationInstructions;
        }
  ...
  validateVariables(componentType, variables) {
    ...
    if (
      variables[nameField] &&
      !/^[a-z][a-z0-9-]*$/.test(variables[nameField])
    ) {
      errors.push(
        `Invalid naming convention for ${nameField}: must be lowercase with hyphens`
      );
    }
  ...
  getTriggerCondition(type, answers) {
    switch (type) {
      ...
      case 'event':
        return answers.eventTriggers
          ? answers.eventTriggers.join(' OR ')
          : 'system.event';
      case 'webhook':
    ...
    for (const taskInfo of tasksToCreate) {
      try {
        ...
        if (result.success) {
          ...
          console.log(
            chalk.red(`   ✗ Failed to create task: ${taskInfo.taskId}`)
          );
        }
      } catch (error) {
        console.log(
          chalk.red(
            `   ✗ Error creating task ${taskInfo.taskId}: ${error.message}`
          )
        );
      }
    }
  ...
  commandToTaskId(command) {
    ...
    if (
      cleanCommand.startsWith('create-') ||
