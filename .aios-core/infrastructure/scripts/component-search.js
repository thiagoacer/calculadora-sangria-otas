...
class ComponentSearch {
  ...
  async findComponent(componentType, componentName) {
    ...
    try {
      const component = await this.searchComponent(
        componentType,
        componentName
      );
  ...
  async findExactMatch(searchPath, componentName, extension) {
    ...
    for (const filename of possibleNames) {
      ...
      try {
        ...
        const component = await this.createComponentInfo(
          filePath,
          componentName
        );
        return component;
    ...
    try {
      ...
      if (matchingFiles.length > 0) {
        ...
        const bestMatch = matchingFiles.reduce((best, current) => {
          const bestScore = this.calculateSimilarity(
            componentName,
            path.basename(best, extension)
          );
  ...
  async createComponentInfo(filePath, componentName) {
    ...
    const component = {
      ...
      name: componentName,
      filePath: filePath,
      registrationFile: await this.findRegistrationFile(
        componentType,
        componentName
      ),
  ...
  inferComponentType(filePath) {
    for (const [type, config] of Object.entries(this.componentTypes)) {
      if (
        filePath.includes(config.directory) &&
        filePath.endsWith(config.extension)
      ) {
        return type;
      }
    }
