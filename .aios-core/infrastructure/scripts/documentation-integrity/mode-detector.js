...
function detectInstallationMode(targetDir = process.cwd()) {
  ...
  if (markers.hasAiosCore && !markers.isAiosCoreRepo) {
    return {
      ...
      confidence: 95,
      reason:
        'AIOS already installed in user project - treating as brownfield update',
      markers,
    };
  }
...
function isAiosCoreRepository(targetDir) {
  ...
  try {
    ...
    const hasAiosMarker = fs.existsSync(
      path.join(targetDir, '.aios-core', 'infrastructure')
    );
    const hasWorkspaces =
    ...
    if (process.env.AIOS_DEBUG) {
      console.warn(
        `[mode-detector] Error checking aios-core repository: ${error.message}`
      );
    }
    return false;
  }
...
function validateModeSelection(selectedMode, detected) {
  ...
  if (selectedMode !== detected.mode) {
    ...
    if (
      ...
      !detected.markers.isAiosCoreRepo
    ) {
      result.warnings.push(
