...
class DocumentationSynchronizer extends EventEmitter {
  constructor(options = {}) {
    ...
    this.options = {
      ...
      syncStrategies: options.syncStrategies || [
        'jsdoc',
        'markdown',
        'schema',
        'api',
  ...
  async buildDocumentationIndex() {
    ...
    for (const docFile of docFiles) {
      try {
        ...
        const metadata = await this.extractDocumentationMetadata(
          docFile,
          content
        );
  ...
  async extractDocumentationMetadata(filePath, content) {
    ...
    if (ext === '.md') {
      ...
      try {
        ...
        metadata.title =
          data.name || data.title || path.basename(filePath, ext);
  ...
  async findCodeRelationships(docPath, docInfo) {
    ...
    if (
      docPath.includes('agents') ||
      docPath.includes('tasks') ||
      docPath.includes('workflows')
    ) {
  ...
  async synchronizeComponent(componentPath, options = {}) {
    ...
    for (const strategyName of strategies) {
      ...
      try {
        // Detect changes
        const detected = await strategy.detector(
          componentPath,
          component.docPath
        );
  ...
  updateDocumentationSection(docContent, name, jsdoc, docSection) {
    ...
    const updatedSection = this.generateDocumentationSection(
      jsdoc,
      docSection.headerLevel
    );
    ...
    try {
      ...
      if (codePath.endsWith('.md') && codePath.includes('tasks')) {
        ...
        if (
          commandPattern &&
          docCommandPattern &&
          commandPattern !== docCommandPattern
        ) {
          changes.push({
  ...
  updateCodeBlock(docContent, block, newCode) {
    ...
    return (
      docContent.substring(0, block.startIndex) +
      newBlock +
  ...
  async detectSchemaChanges(codePath, docPath) {
    ...
    if (
      !codePath.endsWith('.yaml') &&
      !codePath.endsWith('.yml') &&
      !codePath.endsWith('.json')
    ) {
      return changes;
    }
    ...
    try {
      ...
      if (docPath.endsWith('.md')) {
        ...
        for (const field of schemaFields) {
          const documented = schemaTables.find((t) =>
            t.fields.includes(field.name)
          );
          if (!documented) {
            changes.push({
  ...
  async detectExampleChanges(codePath, docPath) {
    ...
    try {
      ...
      for (const block of codeBlocks) {
        if (
          block.code.includes('// Example') ||
          block.code.includes('// Usage')
        ) {
