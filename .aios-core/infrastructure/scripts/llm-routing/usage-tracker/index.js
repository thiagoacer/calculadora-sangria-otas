...
function calculateCost(model, usage) {
  ...
  const cachedCost =
    ((usage.prompt_cache_hit_tokens || 0) / 1_000_000) * pricing.cached;
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.url.startsWith('/usage/')) {
      ...
      if (aliasData) {
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(
          JSON.stringify({ alias: requestedAlias, ...aliasData }, null, 2)
        );
      } else {
  ...
  if (alias && data.aliases[alias]) {
    ...
    if (Object.keys(a.byModel).length > 0) {
      ...
      for (const [model, stats] of Object.entries(a.byModel)) {
        lines.push(
          `    ${model}: ${stats.requests} req, $${stats.cost.toFixed(4)}`
        );
      }
      lines.push('');
    }
  ...
  switch (command) {
    ...
    case 'serve': {
      const port =
        ...
        DEFAULT_PORT;
      const alias =
        args.find((a) => a.startsWith('--alias='))?.split('=')[1] ||
        'claude-free';
    ...
    case 'export': {
      ...
      const exportFile =
        args[1] ||
        `deepseek-usage-${new Date().toISOString().split('T')[0]}.json`;
