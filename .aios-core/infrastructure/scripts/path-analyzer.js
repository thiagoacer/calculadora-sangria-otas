...
const REFERENCE_PATTERNS = [
  ...
  {
    pattern: /^\s+-\s+([a-z0-9-]+\.(?:md|yaml|yml|js))$/gim,
...
function resolveReference(ref, rootPath) {
  ...
  if (baseDir) {
    ...
    const filename = ref.reference.includes('/')
      ? path.basename(ref.reference)
      : ref.reference;
    ...
    try {
      ...
      for (const file of files) {
        ...
        // Check for similar name (case insensitive)
          ...
          fileWithoutExt.toLowerCase() === nameWithoutExt.toLowerCase()
        ) {
          suggestions.push({
            type: 'similar-name',
            suggestedPath: path.join(dir, file),
...
async function analyzePaths(rootPath, options = {}) {
  ...
  for (const result of results) {
    referenceGraph[result.file] = result.validReferences.map(
      (r) => r.resolvedPath
    );
  }
...
Options:
  ...
  if (rootPath === '/') {
    console.error(
      'Error: Could not find .aios-core directory. Run from project root.'
    );
    process.exit(1);
  }
