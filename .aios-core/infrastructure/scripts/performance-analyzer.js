...
/**
 * Performance bottleneck analyzer for Synkra AIOS framework
...
class PerformanceAnalyzer {
  ...
  async analyzePerformance(components) {
    ...
    try {
      ...
      console.log(
        chalk.gray(
          `   Files analyzed: ${analysis.metrics.total_files_analyzed}`
        )
      );
      console.log(
        chalk.gray(`   Issues found: ${analysis.metrics.total_issues_found}`)
      );
    ...
    try {
      ...
      if (stats.size > this.thresholds.fileSize) {
        analysis.file_size_issues.push({
          ...
          severity:
            stats.size > this.thresholds.fileSize * 2 ? 'high' : 'medium',
      ...
      console.warn(
        chalk.yellow(
          `Failed to analyze performance for ${component.id}: ${error.message}`
        )
      );
    }
        ...
        if (matches) {
          matches.forEach((match) => {
            ...
            if (match.includes('new Array')) {
              severity = 'high';
              recommendation =
                'Use array literals or streaming for large datasets';
            } else if (match.includes('JSON.stringify.*JSON.parse')) {
              recommendation =
                'Use direct object assignment or deep clone utilities';
            } else if (match.includes('.map(.*map(')) {
              recommendation =
                'Combine operations or use more efficient iteration';
            }
        ...
        if (matches) {
          matches.forEach((match) => {
            ...
            if (match.includes('.length')) {
              ...
              severity = 'high';
              recommendation =
                'Add proper exit condition to prevent infinite loops';
            } else if (match.includes('forEach.*forEach')) {
              recommendation =
                'Consider using nested for loops or array methods like flatMap';
            }
    ...
    functions.forEach((func) => {
      ...
      if (func.lines > this.thresholds.functionLength) {
        issues.push({
          ...
          line: func.startLine,
          severity:
            func.lines > this.thresholds.functionLength * 2 ? 'high' : 'medium',
      ...
      if (func.complexity > this.thresholds.cyclomaticComplexity) {
        issues.push({
          ...
          line: func.startLine,
          severity:
            func.complexity > this.thresholds.cyclomaticComplexity * 1.5
              ? 'high'
              : 'medium',
  ...
  analyzeImports(content, component) {
    ...
    if (imports.length > this.thresholds.importCount) {
      issues.push({
        ...
        severity: 'medium',
        recommendation:
          'Consider breaking down module or using dynamic imports',
        impact: 'low',
        effort: 'medium',
      });
    }
      ...
      if (!func.name.startsWith('_') && !exports.includes(func.name)) {
        ...
        if (usage <= 1) {
          ...
          issues.push({
            ...
            severity: 'low',
            recommendation:
              'Remove if truly unused or export if needed elsewhere',
            impact: 'low',
            effort: 'low',
          });
        }
  ...
  calculateIssuePriorities(analysis) {
    ...
    metrics.critical_issues = allIssues.filter(
      (i) => i.severity === 'critical'
    ).length;
    metrics.high_priority_issues = allIssues.filter(
      (i) => i.severity === 'high'
    ).length;
    metrics.medium_priority_issues = allIssues.filter(
      (i) => i.severity === 'medium'
    ).length;
    metrics.low_priority_issues = allIssues.filter(
      (i) => i.severity === 'low'
    ).length;
