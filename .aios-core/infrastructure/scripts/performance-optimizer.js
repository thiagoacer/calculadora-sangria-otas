...
class PerformanceOptimizer extends EventEmitter {
  ...
  async analyzePerformance(filePath, options = {}) {
    ...
    try {
      ...
      const patterns =
        options.patterns || Array.from(this.optimizationPatterns.keys());
            ...
            for (const issue of issues) {
              ...
              if (suggestion) {
                analysis.suggestions.push({
                  pattern: patternName,
                  issueId:
                    issue.id || `${patternName}-${analysis.issues.length}`,
                  ...suggestion,
                });
              }
            }
        ...
        node,
        {
          ...
          CallExpression(path) {
            ...
            if (t.isMemberExpression(path.node.callee)) {
              ...
              if (
                t.isIdentifier(property) &&
                ['sort', 'reverse', 'includes', 'indexOf'].includes(
                  property.name
                )
              ) {
        ...
        path.traverse({
              ...
              if (t.isIdentifier(property)) {
                if (['push', 'unshift'].includes(property.name)) {
                  loopIssues.push({
                    ...
                    description:
                      'Growing array in loop can cause performance issues',
                  });
                }
                ...
                if (
                  ['find', 'filter', 'map', 'reduce'].includes(property.name)
                ) {
                  ...
                  if (parentLoop && parentLoop !== path) {
                    loopIssues.push({
                      ...
                      method: property.name,
                      description:
                        'Nested iteration can lead to O(n²) complexity',
                    });
                  }
                }
              }
          ...
          BinaryExpression(innerPath) {
            ...
            if (
              ...
              (t.isStringLiteral(innerPath.node.left) ||
  ...
  async suggestLoopOptimizations(issue) {
    ...
    return {
      optimizations,
      priority: issue.problems.some((p) => p.type === 'nested_iteration')
        ? 'high'
        : 'medium',
    };
  }
          ...
          if (t.isIdentifier(property)) {
            ...
            if (property.name === 'concat' && isInLoop(path)) {
              issues.push({
                ...
                location: path.node.loc,
                description:
                  'Concatenating arrays in loop creates many intermediate arrays',
              });
            }
          }
        }
        ...
        if (this.isPureFunction(path)) {
          ...
          if (complexity > 5) {
            issues.push({
              ...
              location: path.node.loc,
              description:
                'Pure function with high complexity could benefit from memoization',
            });
          }
        }
      },
  ...
  isSimilarCall(call1, call2) {
    ...
    return (
      this.getFunctionCallSignature(call1) ===
      this.getFunctionCallSignature(call2)
    );
  }
      ...
      CallExpression(path) {
        ...
        if (t.isMemberExpression(callee)) {
          ...
          if (t.isIdentifier(property)) {
            ...
            if (
              ['find', 'findOne', 'findById', 'query', 'get'].includes(
                property.name
              )
            ) {
      ...
      CallExpression(path) {
        ...
        if (
          ...
          if (inLoop) {
            issues.push({
              ...
              location: path.node.loc,
              description:
                'Multiple setState calls in loop cause unnecessary re-renders',
            });
          }
        }
        ...
        if (
          ...
          if (funcParent && this.isReactComponent(funcParent)) {
            ...
            const hasReactMemo =
              funcParent.parent?.callee?.property?.name === 'memo';
          ...
          if (
            ...
            if (
              ...
              issues.push({
                ...
                location: attr.loc,
                description:
                  'Inline function prop causes unnecessary re-renders',
              });
            }
          }
        }
      ...
      CallExpression(path) {
        if (t.isMemberExpression(path.node.callee)) {
          ...
          if (t.isIdentifier(property)) {
            ...
            if (
              ['split', 'replace', 'substring', 'substr'].includes(
                property.name
              )
            ) {
              const parent = path.getFunctionParent();
  ...
  estimatePerformanceImprovement(issue) {
    ...
    if (issue.type === 'high_complexity' && issue.complexity.notation) {
      return (
        improvements.high_complexity[issue.complexity.notation] ||
  ...
  generateRecommendations(report) {
    ...
    if (report.byCategory.async?.count > 10) {
      recommendations.push({
        category: 'async',
        recommendation:
          'Review async/await patterns and consider using Promise.all',
        priority: 'medium',
      });
    }
