...
class TemplateValidator {
    ...
    switch (templateType) {
      case 'agent':
        ...
        if (!template.includes('agent:') || !template.includes('persona:')) {
          errors.push(
            'Agent template must include agent: and persona: sections'
          );
        }
      ...
      case 'workflow':
        ...
        if (!template.includes('workflow:') || !template.includes('steps:')) {
          errors.push(
            'Workflow template must include workflow: and steps: sections'
          );
        }
  ...
  checkSecurityIssues(template) {
    ...
    const dangerousPatterns = [
      {
        pattern: /eval\s*\(/,
        message: 'Template contains eval() - security risk',
      },
      {
        pattern: /Function\s*\(/,
        message: 'Template contains Function() - security risk',
      },
      {
        pattern: /require\s*\([^'"]+\)/,
        message: 'Dynamic require detected - potential security risk',
      },
