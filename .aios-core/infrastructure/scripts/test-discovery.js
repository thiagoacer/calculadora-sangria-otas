    ...
    for (const [frameworkId, config] of Object.entries(FRAMEWORKS)) {
      ...
      if (packageJson && config.packageKey) {
        ...
        if (packageJson.scripts) {
          ...
          if (
            scriptsStr.includes(config.packageKey) ||
            scriptsStr.includes(frameworkId)
          ) {
      ...
      if (
        frameworkId === 'go' &&
        fs.existsSync(path.join(this.rootPath, 'go.mod'))
      ) {
...
class TestAnalyzer {
  ...
  analyzeJavaScript(content, analysis) {
    ...
    const focusMatches = content.matchAll(
      /(?:test|it|describe)\.only\s*\(|fit\s*\(/g
    );
  ...
  analyzePython(content, analysis) {
    ...
    // Check for skipped tests
    const skipMatches = content.matchAll(
      /@pytest\.mark\.skip|@unittest\.skip/g
    );
    analysis.skipped = [...skipMatches].length;
  ...
  analyzePHP(content, analysis) {
    ...
    const annotationMatches = content.matchAll(
      /@test[\s\S]*?function\s+(\w+)\s*\(/g
    );
  ...
  analyzeJava(content, analysis) {
    ...
    const testMatches = content.matchAll(
      /@Test[\s\S]*?(?:public|void)\s+(?:void\s+)?(\w+)\s*\(/g
    );
    for (const match of testMatches) {
      analysis.tests.push(match[1]);
    }
...
class CoverageAnalyzer {
  ...
  async analyze() {
    ...
    const jestConfig = this.readConfig([
      'jest.config.js',
      'jest.config.ts',
      'jest.config.json',
    ]);
    ...
    const nycConfig = this.readConfig([
      '.nycrc',
      '.nycrc.json',
      'nyc.config.js',
    ]);
    ...
    const pytestConfig = this.readConfig([
      'pytest.ini',
      'setup.cfg',
      'pyproject.toml',
    ]);
...
class TestRunner extends EventEmitter {
  ...
  parseOutput(output) {
    ...
    // Go format
    const goMatch = output.match(
      /ok\s+.*?\s+([\d.]+)s|FAIL\s+.*?\s+([\d.]+)s/g
    );
    if (goMatch) {
  ...
  async scan(options = {}) {
    ...
    try {
      ...
      if (result.primaryFramework) {
        result.testFiles = await this.finder.findTests(
          result.primaryFramework.id
        );
      } else {
