...
class VisualImpactGenerator {
  ...
  async generateImpactVisualization(impactReport, options = {}) {
    ...
    try {
      ...
      switch (config.format) {
        case 'visual':
        case 'ascii':
          visualization.asciiGraph = await this.generateAsciiVisualization(
            impactReport,
            config
          );
          break;
        case 'html':
          visualization.htmlContent = await this.generateHtmlVisualization(
            impactReport,
            config
          );
          visualization.asciiGraph = await this.generateAsciiVisualization(
          ...
          break;
        case 'json':
          visualization.jsonData = await this.generateJsonVisualization(
            impactReport,
            config
          );
          break;
      ...
      visualization.relationshipMap = await this.generateRelationshipMap(
        impactReport,
        config
      );

      // Generate risk heatmap
      visualization.riskHeatmap = this.generateRiskHeatmap(
        impactReport,
        config
      );
      ...
      this.generationHistory.push({
        ...
        format: config.format,
        componentsVisualized:
          impactReport.dependencyAnalysis.affectedComponents.length,
  ...
  async generateAsciiVisualization(impactReport, config) {
    ...
    const riskIcon =
      template.riskLevels[impactReport.riskAssessment.overallRisk] || '⚪';

    lines.push(
      `${targetIcon} Target Component: ${impactReport.targetComponent.path}`
    );
    lines.push(
      `${riskIcon} Risk Level: ${impactReport.riskAssessment.overallRisk.toUpperCase()}`
    );
    ...
    if (impactReport.propagationAnalysis.criticalPaths?.length > 0) {
      ...
      impactReport.propagationAnalysis.criticalPaths
        .slice(0, 3)
        .forEach((path, index) => {
          lines.push(
            `Path ${index + 1}: Impact ${path.totalImpact}/10, Depth ${path.maxDepth}`
          );
          if (path.effects && path.effects.length > 0) {
            path.effects.slice(0, 3).forEach((effect) => {
              const componentIcon = this.getComponentIcon(
                effect.affectedComponentType
              );
...
<body>
  ...
  generateImpactSummary(impactReport) {
    return {
      ...
      distribution: {
        ...
        impact_categories:
          this.calculateImpactCategoryDistribution(impactReport),
      },
    };
  }
  ...
  generateRiskHeatmap(impactReport, config) {
    ...
    if (impactReport.riskAssessment.riskDimensions) {
      Object.entries(impactReport.riskAssessment.riskDimensions).forEach(
        ([dimension, data]) => {
  ...
  buildDependencyTree(affectedComponents) {
    ...
    const byImpact = {
      ...
      high: affectedComponents.filter(
        (c) => c.impactScore >= 7 && c.impactScore < 9
      ),
      medium: affectedComponents.filter(
        (c) => c.impactScore >= 4 && c.impactScore < 7
      ),
  ...
  renderDependencyTree(tree, config, depth = 0, isLast = true) {
    ...
    if (depth === 0) {
      ...
      const icon = tree.level
        ? this.getRiskIcon(tree.level)
        : this.getComponentIcon(tree.type);
    ...
    if (tree.children) {
      tree.children.forEach((child, index) => {
        ...
        const childLines = this.renderDependencyTree(
          child,
          config,
          depth + 1,
          childIsLast
  ...
  generateTimelineData(impactReport) {
    ...
    if (impactReport.riskAssessment.riskProjection?.risk_timeline) {
      impactReport.riskAssessment.riskProjection.risk_timeline.forEach(
        (event) => {
          timeline.push({
            time: event.time,
            event: event.event,
  ...
  calculateRiskMatrix(impactReport) {
    ...
    const dimensions = Object.keys(
      impactReport.riskAssessment.riskDimensions || {}
    );
    const components = impactReport.dependencyAnalysis.affectedComponents.slice(
      0,
      10
    );
    ...
    components.forEach((component) => {
      ...
      dimensions.forEach((dimension) => {
        ...
        row.scores[dimension] = Math.min(
          10,
          component.impactScore + Math.random() * 2
        );
      });
