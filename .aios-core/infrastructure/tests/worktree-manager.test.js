  ...
  describe('list', () => {
    ...
    it('should return created worktrees', async () => {
      try {
        ...
        expect(worktrees.map((w) => w.storyId).sort()).toEqual([
          'STORY-1',
          'STORY-2',
        ]);
      } catch (error) {
  ...
  describe('detectConflicts', () => {
    ...
    it('should detect conflicting files', async () => {
      try {
        ...
        await execa('git', ['add', '.'], { cwd: worktreePath });
        await execa('git', ['commit', '-m', 'Modify README in worktree'], {
          cwd: worktreePath,
        });
  ...
  describe('mergeToBase', () => {
    ...
    it('should merge worktree successfully', async () => {
      try {
        ...
        await execa('git', ['add', '.'], { cwd: worktreePath });
        await execa('git', ['commit', '-m', 'Add feature'], {
          cwd: worktreePath,
        });
    ...
    it('should support staged option (--no-commit)', async () => {
      try {
        ...
        await execa('git', ['add', '.'], { cwd: worktreePath });
        await execa('git', ['commit', '-m', 'Add staged feature'], {
          cwd: worktreePath,
        });
    ...
    it('should fail gracefully on conflicts', async () => {
      try {
        ...
        await execa('git', ['add', '.'], { cwd: worktreePath });
        await execa('git', ['commit', '-m', 'Modify in worktree'], {
          cwd: worktreePath,
        });
        ...
        await execa('git', ['add', '.'], { cwd: testRoot });
        await execa('git', ['commit', '-m', 'Modify in main'], {
          cwd: testRoot,
        });
    ...
    it('should cleanup worktree after merge when cleanup option is true', async () => {
      try {
        ...
        await execa('git', ['add', '.'], { cwd: worktreePath });
        await execa('git', ['commit', '-m', 'Add cleanup test'], {
          cwd: worktreePath,
        });
  ...
  describe('getMergeHistory', () => {
    ...
    it('should return merge history after merge', async () => {
      try {
        ...
        await execa('git', ['add', '.'], { cwd: worktreePath });
        await execa('git', ['commit', '-m', 'Add history test'], {
          cwd: worktreePath,
        });
