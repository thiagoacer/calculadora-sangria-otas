...
if (npmVersion) {
  ...
  console.log(
    `  ${checkMark(npmOk)} Version:     v${npmVersion} (required: >= v${REQUIRED_NPM})`
  );
  if (!npmOk) {
...
if (registryCheck && registryCheck.includes('PONG')) {
  ...
  // Try curl/wget
  const curlCheck = exec(
    'curl -s -o /dev/null -w "%{http_code}" https://registry.npmjs.org/ 2>&1'
  );
  if (curlCheck === '200') {
...
if (os.platform() === 'win32') {
  ...
  if (psPolicy) {
    const policyOk = ['Unrestricted', 'RemoteSigned', 'Bypass'].includes(
      psPolicy
    );
    console.log(
      `  ${checkMark(policyOk)} PowerShell Execution Policy: ${psPolicy}`
    );
    if (!policyOk) {
...
if (hasErrors) {
  ...
  console.log(
    '  1. Update Node.js: https://nodejs.org/ (download LTS version)'
  );
