...
class TestTemplateSystem {
  ...
  async initialize() {
    try {
      ...
      for (const _framework of Object.keys(this.frameworks)) {
        await fs.mkdir(path.join(this.templatesDir, framework), {
          recursive: true,
        });
      }
      ...
      console.error(
        chalk.red(`Failed to initialize test template system: ${error.message}`)
      );
      throw error;
    }
  }
  ...
  async createCustomTemplate(templateName, templateContent, options = {}) {
    ...
    try {
      ...
      await fs.writeFile(
        templatePath,
        JSON.stringify(templateWrapper, null, 2)
      );
      ...
      console.error(
        chalk.red(`Failed to create custom template: ${error.message}`)
      );
      throw error;
    }
  }
  ...
  async generateTemplate(componentType, testType, _framework, options = {}) {
    const _baseTemplate = this.getBaseTemplate(
      componentType,
      testType,
      framework
    );
    ...
    const template = {
      ...
      testCases: this.generateTestCaseTemplates(
        componentType,
        testType,
        options
      ),
      ...
      utilities: this.generateUtilityTemplates(
        componentType,
        testType,
        _framework,
        options
...
// Framework: ${data.config.framework}`;
  ...
  replaceTemplateVariables(content, sections) {
    ...
    result = result.replace(
      /\{\{componentClass\}\}/g,
      '${this.toClassName(data.component.name)}'
    );
