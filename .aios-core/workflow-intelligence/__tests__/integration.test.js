  ...
  describe('End-to-end workflow navigation', () => {
    it('should navigate through epic creation workflow', () => {
      ...
      expect(
        suggestions1.some(
          (s) =>
            s.command === 'validate-story-draft' ||
            s.command === 'create-next-story'
        )
      ).toBe(true);
      ...
      expect(
        suggestions2.some(
          (s) => s.command === 'analyze-impact' || s.command === 'develop'
        )
      ).toBe(true);
    });
  ...
  describe('All 10 workflows have transitions', () => {
    ...
    workflows.forEach((workflowName) => {
      it(`${workflowName} should have transitions defined`, () => {
        ...
        expect(Object.keys(workflow.transitions).length).toBeGreaterThanOrEqual(
          2
        );
      });
