...
function displaySuggestions(result) {
  ...
  output.push('');
  output.push(
    colorize('  Workflow: ', 'cyan') + formatWorkflowName(result.workflow)
  );
  ...
  if (result.suggestions && result.suggestions.length > 0) {
    ...
    result.suggestions.forEach((suggestion, index) => {
      ...
      const args = suggestion.args
        ? ` ${colorize(suggestion.args, 'gray')}`
        : '';
      const desc = suggestion.description
        ? colorize(` - ${suggestion.description}`, 'dim')
        : '';
    ...
    if (result.isUncertain) {
      output.push(
        colorize(
          '  Low confidence - context is unclear. Try providing --story flag.',
          'yellow'
        )
      );
      output.push('');
    ...
    output.push(
      colorize('Unable to determine workflow from current context.', 'yellow')
    );
    output.push('');
    output.push('Try:');
...
function displayHelp() {
  ...
  output.push(
    `  ${colorize('--story <path>', 'cyan')}  Explicit story path for context`
  );
  output.push(
    `  ${colorize('--all', 'cyan')}           Show all suggestions (not just top 3)`
  );
  output.push(
  ...
  output.push(
    `  ${colorize('*next --story docs/stories/v2.1/sprint-10/story-wis-3.md', 'green')}`
  );
  output.push(
...
function displayContext(context) {
  ...
  output.push(
    `  ${colorize('Agent:', 'cyan')} @${context.agentId || 'unknown'}`
  );
  output.push(
    `  ${colorize('Last Command:', 'cyan')} ${context.lastCommand || 'none'}`
  );
